...
Logistische Regression ist eine Erscheinungsform des \textbf{verallgemeinerten linearen Modells}
...
erfahrungsgemäss Schwierigkeiten zubereitet, behandelt dieses Kapitel die wichtigsten Prinzipien
logistischer Regression, auch wenn diese für mich nicht mehr zu den
Grundlagen gehören.
...
Um dieses Problem zu lösen, kann man das
...
erklärt, was das bedeutet. Was die Einstellung
\texttt{link = "logit"} bedeutet wird in Kürze erklärt.

\medskip

\begin{framed}
\textbf{Binomialverteilungen.}
...
<<echo = FALSE, fig.width = 9, fig.height = 1.5*3.5, fig.cap = "\\textit{Obere Zeile:} Drei Binomialverteilungen mit $n = 10$. \\textit{Untere Zeile:} Drei Binomialverteilungen mit $n = 83$.\\label{fig:binomialdistribution}">>=
size <- 10
prob <- 0.5
...
Wahrscheinlichkeitsquoten:\footnote{Für $z > 0$ und $x > 0, x \neq 1$ gilt: Wenn $x^y = z$, ist $\log_x z = y$. 
Beispiele: $10^2 = 100$, also $\log_{10} 100 = 2$;
$2^5 = 32$, also $\log_2 32 = 5$.
$\ln$ ist die Kürzel für den sog.\ natürlichen Logarithmus, d.h., $\log_{e}$, wo $e \approx 2.71828$.}
\begin{equation}\label{eq:logodds}
\textrm{log-odds} = \ln \textrm{odds} = \ln \frac{\textrm{Proportion A}}{\textrm{Proportion B}} = \ln \frac{\textrm{Proportion A}}{1 - \textrm{Proportion A}}
\end{equation}
Diese Funktion -- die Proportionen zu log-odds transformiert --
heisst die logit-Funktion: $\textrm{logit}(x) = \ln \frac{x}{1-x}$,
...
<<echo = FALSE, fig.width = 7, fig.height = 5, fig.cap = "Odds, log-odds und Wahrscheinlichkeiten zueinander konvertieren.\\label{fig:logodds}">>=
...
\begin{itemize}
 ...
 dies ist die Umkehrfunktion der logarithmen Funktion:
<<>>=
exp(0.93)
@
...
entsprechen eine Wahrscheinlichkeit von etwa 0.72.
...
dass eine Versuchsperson in der \textit{gain}-Framing-Kondition
...
0.11 so gross ist als die Chance, dass sich jemand in der
...
Das Chancenverhältnis ist wohl die wichtige Erkenntniss
...
können nicht direkt aus diesem Modell abgeleitet werden.
...
ausrechnen. Das Resultat ist ein 95\%-Konfidenzintervall von [-60, -50] Prozent\-punkten.
<<>>=
confint(tversky.lm)
@
...
\begin{framed}
\textbf{Prozente und Prozentpunkte.}
...
beim \textit{loss}-Framing 50\% liegt, auch so interpretieren,
...
Die Anzahl Wahlen für Programm A vs.\ Programm B zu ermitteln,
ist mittlerweile ein Leichtes.
...
<<fig.width = 3, fig.height = 2, eval = TRUE, fig.cap="Präferenz für das Programm, das Sicherheit bietet (Programm A), je nach Formulierung und Sprache.\\label{fig:keysar}">>=
# Grafik: 2. Versuch
ggplot(d_summary,
       aes(x = Formulierung, y = prop_sicher,
           linetype = Sprache, shape = Sprache,
...
Ich runde dieses Zwischenergebnis hier ab nicht, sodass das Endergebnis nicht
...
<<echo = FALSE, fig.width = 5, fig.height = 4.5, fig.cap="Ob man schlussfolgert, dass eine Interaktion vorliegt, hängt in diesen drei Beispielen davon ab, wie man die Daten ausdrückt: als Proportionen, als Odds (Chancen) oder als Log-Odds.\\label{fig:interactionlogistic}">>=
df <- expand.grid(
  AB = c("A", "B"),
  CD = c("C", "D")
)
...
\begin{framed}
\textbf{Interaktionen in verallgemeinerten linearen Modellen.}
Interaktionen \emph{theoretisch} zu interpretieren
...
zeigt drei Mal das gleiche Datenmuster, aber jeweils anders ausgedrückt.
Ausgedrückt in Proportionen sieht man zwei parallele Linien
(rot: 0.475--0.525, blau: 900--950), die zeigen, dass der Unterschied
...
es auf Englisch vorgelegt wird. Das 95\%-Konfidenzintervall zeigt aber,
...
<<fig.width = 3, fig.height = 2, fig.cap = "Der modellierte Zusammenhang zwischen der orthografischen Überlappung und der Wahrscheinlichkeit, zu der ein Wort richtig übersetzt wurde.\\label{fig:logisticmodel}">>=
...
\begin{framed}
\textbf{Nicht-Linearitäten nicht überinterpretieren!}
