...
zur Urne zu gehen. Öfters ist die Population von Interesse
aber eher als ein abstrakterer datengenerierender Mechanismus
...
wird am Ende dieses Kapitels sowie auch in den folgenden Kapiteln näher
...
\section{Die zentrale Tendenz und Streuung der Population anhand einer Zufallsstichprobe schätzen}
...
das Populationsmittel. Man sagt auch, dass der `Erwartungswert'
des Stichprobenmittels gleich dem Populationsmittel ist.
Das Stichprobenmittel (Kürzel: $\bar{x}$)
ist also eine sog.\ `unverzerrte' (\textit{unbiased}) Schätzung des
...
In unserem Fall also $\sigma^2 = \frac{(5-(-5))^2}{12} \approx 8.33$.
...
Formel \ref{eq:popvar} liefert also eine verzerrte Schätzung
...
(Kürzel: $s^2$) \emph{nicht} wie die Populationsvarianz ($\sigma^2$),
...
\emph{im Schnitt} dem Populationsmittel gleich. Die einzelnen
...
\begin{enumerate}
\item Kopieren Sie den Code auf Seite \pageref{code:stichprobenmittel}
...
<<fig.cap = "Drei Populationen, aus denen hier Stichproben generiert werden.\\label{fig:parentpopulation}", echo = FALSE, fig.width = 8, fig.height = 2>>=
...
\emph{ungefähr normalverteilt}, wenn $n$ gross genug ist.
Dies gilt auch dann,
\emph{wenn die Population selber nicht normalverteilt ist.}
...
Stichprobengrösse abnimmt -- sind von grösster Bedeutung
für die Inferenzstatistik und für das Planen von
Datenerhebungen, wie wir später sehen werden.
...
Impräzise an diesem Satz mag ausserdem erscheinen,
dass er besagt,
...
Tragen Sie diese Daten wie folgt in R ein; die 
\texttt{c()}-Funktion knüpft mehrere Werte 
zusammen (\textit{c} für \textit{concatenate}):
