...
Interessanter wäre die Frage, ob der Effekt von Wortfrequenz unterschiedlich gross ist je nach der Lesefähigkeit der Versuchspersonen.
...
% In den Grafiken links und rechts oben sowie links unten gibt es 
% also Haupteffekte von sowohl Lesefähigkeit als auch von Wortfrequenz 
% auf Lesegeschwindigkeit: Gemittelt über die Ausprägungen von Lesefähigkeit 
% werden hochfrequente Wörter schneller gelesen als Wörter mit niedriger 
% Frequenz. Ebenso lesen beschlagene Lesende, gemittelt über die Ausprägungen 
% von Wortfrequenz, schneller als schlechtere Lesende.
...
% \paragraph{Aufgabe 1} Zeichnen Sie (von Hand) ein Liniendiagramm, 
% einen Haupteffekt von Wortfrequenz zeigt, nicht jedoch von Lesefähigkeit.

% \paragraph{Aufgabe 2} Die \textit{cross-over}-Interaktion in Abbildung 
% \ref{fig:interactions} zeigt zwar keine Haupteffekte, aber 
% \textit{cross-over}-Interaktionen können durchaus vorkommen, 
% auch wenn Haupteffekte vorliegen. Zeichnen Sie (von Hand) ein Liniendiagramm, 
% das eine \textit{cross-over}-Interaktion zwischen Wortfrequenz und 
% Lesefähigkeit zeigt sowie auch mindestens einen Haupteffekt.
...
schweizerischen Namen hat?\footnote{Siehe abermals Fussnote \vref{fn:continuous}.}
...
Mit \texttt{facet\_grid(. \textasciitilde  CS)} wird die Grafik horizontal in zwei Teile aufgespaltet: einen
für die Fälle, in denen Codeswitching vorlag, und einen für Fälle, in denen dies nicht der Fall war.
Wie Abbildung \ref{fig:berthelebp1} aber zeigt, dürften diese Daten etwas zu grobkörning sein,
...
<<fig.width = 3, fig.height = 1.8, fig.cap = "Liniendiagramm mit den Mitteln der Bewertungen des akademischen Potenzials des Sprechers je nach Vorkommen von Codeswitches (mit vs.\\ ohne) und seinem angeblichen Namen. Die Muster sind hier deutlicher, aber dieser Grafik kann der Streuung der Daten nicht entnommen werden.\\label{fig:berthelebp2}">>=
...
<<fig.width = 5, fig.height = 2.2, fig.cap = "Bewertungen des akademischen Potenzials des Sprechers je nach Vorkommen von Codeswitches (mit vs.\\ ohne) und seinem angeblichen Namen. Die Sternchen stellen die Gruppenmittel dar.\\label{fig:berthelebp3}">>=
ggplot(d,
       aes(x = Name,
           y = Potenzial)) +
...
\begin{framed}
\textbf{Probieren Sie mehrere Grafiken aus.} Für
...
nicht deutlich genug hervorhoben. Der Einstellung
\texttt{size = 3} liegt eine ähnliche Beobachtung zu Grunde,
usw.
\end{framed}
...
Dragan als jenen von Luca beeindruckt.
...
\paragraph{Denkaufgabe} Warum haben sich die Parameterschätzungen geändert?
...
ist etwas heikel: Wegen des \textit{treatment codings} bezieht sich die Schätzung von $0.53$ für \texttt{Dragan}
...
<<fig.width = 5, fig.height = 2.2, fig.cap = "Zellenmittel und modellbasierte 95\\%-Konfidenzintervalle.\\label{fig:bertheleCI}">>=
ggplot(neue_daten,
       aes(x = Name,
           y = Mittel)) +
  # geom_point(data = d,
...
Da auch des Modell, mit dem wir uns gerade amüsiert haben, ein Beispiel eines allgemeinen linearen Modells ist,
...
Homoskedastizitätsannahmen eingeschätzt werden können, siehe Abbildung \ref{fig:diagnosticpotenzial}.

<<echo = FALSE, fig.width = 4, fig.height = 4, fig.cap = "Diagnostische Plots für das \\texttt{potenzial.lm}-Modell.\\label{fig:diagnosticpotenzial}", fig.pos="tp">>=
...
des allgemeinen linearen Modell. Aber wir können dies auch verhindern,
...
<<fig.width = 8, fig.height = 4, fig.cap="Bootstrapschätzungen der Variabilität der Modellparameter ohne Homoskedastizitätsannahme.\\label{fig:bertheleBS}">>=
bs_beta_tbl <- tibble(
...
\section{Interaktionen zwischen einem kontinuierlichen und einem binären Prädiktor}\label{sec:interactioncontinuous}
...
\textbf{Lieber ein einziges Modell als viele kleine.}
...
behandeln \emph{und} sinnvoll mit dem allgemeinen linearen Modell analysiert werden
...
<<fig.width = 4, fig.height = 1.8, fig.cap = "Ups.\\label{fig:oops}">>=
ggplot(d,
...
<<fig.width = 4, fig.height = 1.8, fig.cap = "Für sowohl Männer als auch Frauen gibt es einen positiven Zusammenhang zwischen der Leistung beim Englischtest und bei der Kognatübersetzung.\\label{fig:engl}">>=
...
<<fig.width = 5, fig.height = 2, fig.cap = "Visualisierung des \\texttt{kognat.lm}-Modells mit 80\\%- und 95\\%-Konfidenzbändern.\\label{fig:effectkognat}">>=
...
<<eval = TRUE, echo = FALSE, fig.width = 4, fig.height = 2, fig.cap = "In diesem simulierten Datensatz gibt es zwar Haupteffekte von A vs.\\ B und von X vs.\\ Y, aber eigentlich schneidet nur die Gruppe mit A \\emph{und} Y wesentlich besser als die anderen ab.\\label{fig:maineffectinteraction}">>=
set.seed(14-05-2015)
  F1 <- c(rnorm(40, 40, 20),
          rnorm(40, 60, 20),
          rnorm(40, 40, 20),
