...
Im letzten Kapitel haben wir uns mit der Frage
...
Gruppen hinsichtlich einer mehr oder weniger kontinuierlichen
outcome zu vergleichen, ist der Boxplot
...
vom 25.\ oder 75.\ entfernt liegen.
Diese Extremwerte sind \emph{mögliche} (!) Ausreisser.
...
<<fig.width = 2, fig.height = 2.1, fig.cap = "Vergleich der Systemrechtfertigungsscores in den beiden Konditionen in Klein et al.'s (2014) Replikation von Caruso et al. (2013, Experiment 1). Daten aus der Abington-Stichprobe.\\label{fig:boxplotklein1}">>=
p_boxplot <- ggplot(data = d,
                    aes(x = MoneyGroup,
                        y = Sysjust)) +
  geom_boxplot() +
  xlab("Kondition") +
  ylab("Systemrechtfertigungs-\nscore")
...
<<fig.width = 2.2, fig.height = 2.1, fig.cap = "Nochmals die gleichen Daten, aber mit den einzelnen Datenpunkten.\\label{fig:boxplotklein2}">>=
...
Beide Möglichkeiten liefern das gleiche Ergebnis; die Idee hier ist nur, die Funktionen 
\texttt{mutate()} und \texttt{case\_when()} vorzustellen.
<<>>=
d <- d %>% 
...
Zur Kontrolle ist eine Kreuztabelle der ursprünglichen und der Dummy-Variablen
nützlich:
...
$\widehat{\beta_1}$. Ihre Bedeutung kann aus der Regressionsgleichung abgeleitet werden:
...
sodass $\widehat{y_i} = 3.53 - 0.006$. Durch Rundungsfehler ergibt dies eigentlich auch 3.53. $\widehat{\beta_1}$ ist also der \emph{Unterschied} zwischen den Mitteln der beiden Gruppen. Ist dieser Wert negativ, dann hat die Gruppe, die als $1$ bezeichnet wurde, ein niedrigeres Mittel als die Gruppe, die als $0$ bezeichnet wurde.
...
<<echo = TRUE, fig.width = 6, fig.height = 2, fig.cap = "Verteilung der Bootstrap-Schätzungen der Parameter im Regressionsmodell \\texttt{money.lm}.\\label{fig:bootstrapdistributionmoney}">>=
...
von \citet{Caruso2013} nicht. Die Bootstrapmethoden liefern ein nahezu identisches Ergebnis.\footnote{
An dieser Stelle sei darauf hingewiesen, dass der outcome in dieser Studie strikte 
genommen keine kontinuierliche Variable ist, sondern das Mittel ordinalskalierter Beobachtungen. 
\citet{Liddell2018} und \citet{Buerkner2019} argumentieren, dass das allgemeine lineare Modell 
gar nicht geeignet ist, um solche Daten auszuwerten. Sagen Sie mir daher bitte Bescheid,
wenn Sie über einen Datensatz verfügen, in dem zwei Gruppen hinsichtlich ihres Mittels bei einer
\emph{echten} kontinuierlichen Variablen verglichen werden, ohne dass weitere Prädiktoren berücksichtigt
werden müssen -- denn ich habe keinen solchen Datensatz, der als Beispiel dienen kann.\label{fn:continuous}}
...
Meines Erachtens pflegen Sie besser die Gewohnheit,
...
\begin{enumerate}
  ...
  In diesem Fall heisst dies, dass die outcome-Werte \emph{innerhalb}
...
<<echo = FALSE, fig.cap = "Beispiel von heteroskedastischen Daten.\\label{fig:heteroskedasticity_groups}">>=
...
<<fig.width = 6, fig.height = 2.3, fig.cap = "Boxplots der Daten von \\citet{Vanhove2018}.\\label{fig:vanhove2018}">>=
ggplot(d,
       aes(x = Condition,
...
bei der ersten Dummy-Variablen dar (z.B.\ \texttt{Information}: 0 oder 1);
$x_{2,i}$ stellt ihren Wert bei der zweiten Dummy-Variablen dar (z.B.\ \texttt{Strategy}: 0 oder 1).
...
gedient haben, weshalb wir sie den interessierten LeserInnen als Übung überlassen. 
Die Standardfehler
...
$\widehat{\beta_2}$ den Unterschied zwischen der dritten und der \emph{zweiten} Gruppe
und nicht den Unterschied zwischen der dritten und der \emph{ersten} Gruppe
darstellt. Dies sei hier der Vollständigkeit halber erwähnt;
mehr Informationen finden Sie unter auf der Website
des \href{https://stats.idre.ucla.edu/r/library/r-library-contrast-coding-systems-for-categorical-variables/}{Institute for Digital Research and Education} der UCLA.
...
% 
% \begin{framed}
% Welches Koderierungssystem Sie verwenden sollten, hängt vor allem von Ihrer Forschungsfrage ab.
% Treatment coding ist geeignet, um unterschiedliche Gruppen mit der gleichen Referenzgruppe, die
% vom Intercept dargestellt wird, zu vergleichen.
% \end{framed}

% \begin{mdframed}
% \noindent \textbf{Konditionen direkt mit dem Bootstrap vergleichen.}
% Dem Modelloutput können wir zwar entnehmen,
% wie gross die Unterschiede zwischen einerseits der NoInformation-Kondition
% und andererseits der Information- und der Strategy-Kondition ist und was
% die Konfidenzintervalle um diese Unterschiede sind. Aber was ist der Unterschied
% zwischen der Information- und der Strategy-Kondition und sein Konfidenzintervall?
% 
% Um dies zu berechnen haben wir drei Möglichkeiten:
% 
% \begin{enumerate}
%  \item Wir machen uns bei den oben erwähnten Ressourcen schlau und stellen fest,
%  dass \textit{backward difference coding} vielleicht nützlich wäre.
%  
%  \item Wir kodieren unsere Variablen anders, sodass das Intercept jetzt das Mittel 
%  der Information-Kondition darstellt:
% <<>>=
% d$NoInformation <- ifelse(d$Condition == "no information", 1, 0)
% mod.lm3 <- lm(ProportionCongruent ~ NoInformation + Strategy, data = d)
% coef(mod.lm3)
% confint(mod.lm3)
% @
% 
% Der Unterschied zwischen Information und Strategy ist also 0.11 mit einem
% 95\%-Konfidenzintervall von [$-0.03$, $0.25$].
% 
% \item 
% 
%  
...
\begin{enumerate}
 ...
 \item In Abschnitt \vref{sec:helascot} haben wir mit Datensätzen mit Angaben
 zu den Lesefähigkeiten von Kindern portugiesischer Herkunft gearbeitet. Diesen
 verwenden wir auch in dieser Aufgabe.
 \begin{enumerate}
  ...
  \item Vergleichen Sie die Leistung von Kindern in Portugal mit jener von Kindern in der Romandie
  und in der Deutschschweiz (Variable \texttt{LanguageGroup: Control group Portuguese} vs.\ \texttt{Bilingual group French} bzw.\ \texttt{Bilingual group German}) bei der portugiesischen Leseaufgabe (\texttt{Reading}) zur zweiten Erhebung. 
  ...
  und der letzte Parameter den durchschnittlichen Unterschied zwischen der Romandie und der Deutschschweiz zeigt. Rechnen Sie anhand des Modelloutputs kurz nach, ob die erhaltenen
  Schätzungen auch stimmen. (Tipp: Siehe Abschnitt \ref{sec:kodierungssysteme}, insbesondere den Blogeintrag oder \citet{Schad2018}.)\\
  (Eine solche Kodierung wäre sehr geeignet, wenn Sie sich für diese zwei Forschungsfragen interessieren: (1) Wie stark unterscheidet sich die Leistung von Kindern in Portugal und portugiesischstämmigen Kindern in der Schweiz? (2) Wie stark unterscheidet sich die Leistung von portugiesischstämmigen Kindern in der Schweiz je nach Sprachregion?)
