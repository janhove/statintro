...
beschränke ich mich hier auf zwei Buchempfehlungen:
\begin{itemize}
 \item Bodo Winters \textit{Statistics for linguists: An introduction using R}
schliesst von der Philosophie her gut bei diesem Skript an \citep{Winter2019}.
Er bespricht auch das verallgemeinerte lineare Modell und sog.\ 
...
\begin{itemize}
  ...
  Verfahren vertraut. Versuchen Sie, zu antizipieren, was Sie
  ...
  online verfügbar. Einerseits brauchen Sie so Details,
