\chapter{Wahrscheinlichkeitsaussagen über zufällige Beobachtungen}\label{ch:wahrscheinlichkeiten}
...
$93 - 45 = 48$ und $\frac{48}{360} = 0.133$. 
...
<<fig.cap = "Kumulative Verteilungsfunktion einer kontinuierlichen Gleichverteilung mit Bereich 0 bis 360.\\label{fig:uniformcumulative}", echo = FALSE, fig.height = 2, fig.width = 2.5>>=
...
<<fig.cap = "Wahrscheinlichkeitsdichte und kumulative Wahrscheinlichkeit einer Normalverteilung mit Mittel 100 und Standardabweichung 15.\\label{fig:iqdistribution}", echo = FALSE, fig.height = 2, fig.width = 5>>=
library(RColorBrewer)
...
Oder mithilfe der \texttt{c()}-Funktion (\textit{combine}):
...
Sie mit dem Rechnen mit Wahrscheinlichkeiten vertrauter
zu machen.
...
\item Die nächste Aufgabe gibt es nur, weil es noch Platz auf dem Blatt gab.\\
Ein \textit{straight} besteht aus fünf aufeinander folgenden Werten wie zum Beispiel 8-9-10-Bube-Dame oder 3-4-5-6-7. Die Farben sind dabei unerheblich.
...
Wie gross ist die Wahrscheinlichkeit, dass fünf zufällig gezogene Karten einen \textit{straight} bilden? Beachten Sie, dass es nicht ausmacht, in welcher Reihenfolge man die Karten zieht. 8-4-5-7-6 ist auch ein \textit{straight}, da man mit diesen Karten 4-5-6-7-8 bilden kann.\\
(Tipp: Was ist die Wahrscheinlichkeit, dass man genau Ass-2-3-4-5, in dieser Reihenfolge, zieht? Und in wie vielen Reihenfolgen kann man einen solchen \textit{straight} ziehen?)
