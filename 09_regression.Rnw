...
<<echo = TRUE, fig.cap = "Zusammenhang zwischen AOA und GJT in der Studie von \\citet{DeKeyser2010}.\\label{fig:dekeyser}", fig.width = 4, fig.height = 2.4>>=
ggplot(data = d,
       aes(x = AOA,
           y = GJT)) +
...
<<fig.width = 5.5, fig.height = 1.4, echo = FALSE, fig.cap = "Beispiele von nicht-linearen Zusammenhängen.\\label{fig:nichtlinear}">>=
...
\begin{enumerate}
 ...
 Wie genau `perfekt' in diesem Kontext zu vrstehen ist, sollte in Kürze klar werden.
 (\textbf{Korrelationsanalyse}) 
 % 
 % Anders gesagt, wenn wir den Wert einer der beiden Variablen kennen, 
 % \emph{wie gut} können wir dann anhand den Wert der anderen Variablen schätzen? 
...
\begin{itemize}
 ...
 Die \emph{Korrelation} ist also äusserst stark (Frage 1). Damit wissen wir aber noch nicht, \emph{wie} wir die Temperatur in Grad Fahrenheit berechnen können, wenn wir die Temperatur in Grad Celsius kennen. Eine \emph{Regression}sanalyse würde zeigen, dass wir dazu die folgende Formel anwenden müssten:
...
den Variablen nicht \emph{ungefähr} gerade, dann \emph{kann}
...
technischen Wortschatz (z.B.\ \textit{Korrelation}, \textit{signifikant})
...
positiver Zusammenhang zwischen den beiden
Variablen (je grösser $x$, desto grösser $y$);
ist die Kovarianz negativ, dann gibt es einen negativen
...
\paragraph{Denkaufgabe 1} Seien $x$ und $y$ zwei numerische Variablen.
...
\paragraph{Denkaufgabe 2} Was ist die Kovarianz zwischen $x$ und $y$, wenn
es zwar unterschiedliche $x$-Werte gibt, aber alle $y$-Werte einander gleich sind?

\paragraph{Denkaufgabe 3} Sei $x$ eine numerische Variable. Was berechnen Sie 
...
Salopp gesagt drückt diese Zahl
aus, wie gut der Zusammenhang durch eine gerade Linie
...
Sobald irgendein Wert fehlt, ergibt diese Funktion
das Ergebnis `NA` (\textit{not available}). Eine Möglichkeit
ist dann, die Beobachtungen mit einem oder zwei fehlenden Wertn zu ignorieren:
...
Zum Beispiel wird die Anzahl richtiger Antworten oft zu $r = -1$ mit der Anzahl falscher Antworten korrelieren; auch dies ist wenig spektakulär.
...
Die Form der Geraden ist zwar gleich wie in der linken Grafik, der Korrelationskoeffizient jedoch nicht.
...
die Form der Geraden jedoch nicht.
...
<<fig.width = 3.5, fig.height = 3.5, echo = FALSE, fig.cap = "Korrelationskoeffizienten erzählen einem wenig über die Form eines Zusammenhangs.\\label{fig:correlation}">>=
par(mar = c(3, 4, 2, 1), mgp=c(2,.7,0), tck = -0.01, las = 1, cex = 0.65, cex.main = 1.1)
...
Umgekehrt kann $r$ den Eindruck geben, dass es sich um einen ziemlich starken linearen Zusammenhang handelt, während ein solcher Zusammenhang für die meisten Datenpunkte kaum vorliegt (mittel),
...
Anweisungen, wie Sie dieses Paket installieren können; mit
\texttt{install.packages()} wird es nämlich nicht klappen.
...
<<fig.width = 10, fig.height = 10, echo = TRUE, fig.cap = "Alle sechzehn Zusammenhänge zeigen eine Korrelation von $-0.72$ auf, sehen jedoch zum Teil ganz unterschiedlich aus.\\label{fig:plotr}">>=
library(cannonball)
plot_r(n = 50, r = -0.72)
@
...
\begin{framed}
\textbf{Ein Korrelationskoeffizient kann einer Vielzahl von Zusammenhängen entsprechen.}
...
Ab und zu trifft man Spearmans $\rho$-Koeffizienten (oder manchmal: $r_s$) an.
...
Mal wahrscheinlicher, dass auch $y_2$ grösser als $y_1$ ist als das er kleiner ist.
...
Statt \emph{automatisch} auf $\rho$ oder $\tau$ zurückzugreifen,
...
(Stärke des Zusammenhangs) interessiert,
...
eingestuft. Ich halte dies für weinig sinnvoll,
weshalb ich diese Einstufungen hier nicht
reproduziere. Wer mehr über sie erfahren möchte,
...
der \texttt{plot\_r()}-Funktion aus dem \texttt{cannonball}-Package
einen Datensatz mit den gewünschten Merkmalen und berechnet dann
das Konfidenzintervall um den Korrelationskoeffizienten.
...
% \begin{enumerate}
  ...
  wird, ist die kognitive Kontrolle. Dieses Konstrukt lässt sich nur \emph{indirekt}
  messen, nämlich mithilfe von kognitiven Tests: Die Leistung beim Test \emph{ist}
  ...
  \end{enumerate}
% 
%   \item Kendalls $\hat{\tau}}$ für den AOA--GJT-Zusammenhang
%   beträgt $-0.60$. 
%   $\hat{\tau} = -0.60$ heisst, dass es etwa 4 Mal wahrscheinlicher ist,
%     dass eine beliebige Person mit einem höheren AOA-Wert als eine beliebige
%     andere Person einen niedrigeren GJT-Wert als diese Person hat.
%     
%   \begin{enumerate}
%     \item Die Stichprobe von \citet{DeKeyser2010} ist höchstwahrscheinlich keine
%     Zufallsstichprobe aus der Population der russischsprachigen MigrantInnen in Israel.
%     Erklären Sie, wieso die folgende Behauptung daher vorschnell sein dürfte:
%     \begin{quote}
%       Nimmt man zwei beliebige russischsprachige MigrantInnen in Israel und legt man ihnen denselben
%       Test wie \citet{DeKeyser2010} vor, dann ist es etwa 4 Mal wahrscheinlicher, 
%       dass die/der ältere der beiden bei diesem Test schlechter abschneiden wird?
%     \end{quote}
%     \item Für diese Schätzungen berechnet
%   die \texttt{cor.test()}-Funktion
%   kein Konfidenzintervall.
%   Passen Sie den Bootstrap auf Seite \pageref{sec:r_bootstrap}
%   so an, dass dieser ein 90\%-Kon\-fi\-denz\-inter\-vall für $\hat{\tau}$
%   berechnet.
%     \item Was ist das 90\%-Konfidenzintervall um die Zahl 4?
  % \end{enumerate}


  % \item Eine von vielen Möglichkeiten, um die lexikalische Variation
  % in einem Text zu quantifizieren, besteht darin, die Anzahl Wörter und
  % die Anzahl \emph{unterschiedlicher} Wörter im Text zu zählen, und daraus
  % den sog.\ Guiraud-Index (nach \citealp{Guiraud1954}) zu berechnen:
  %
  % $$\textrm{Guiraud-Index} = \frac{\textrm{Anzahl unterschiedlicher Wörter}}{\sqrt{\textrm{Anzahl Wörter}}}$$
  %
  % Auch wenn man immer über ein paar Zweifelsfälle diskutieren kann,
  % ist es in der Regel ziemlich einfach, die Anzahl Wörter in einem Text
  % zu berechnen. Bei der Anzahl unterschiedlicher Wörter gibt es jedoch
  % ein grösseres Diskussionspotenzial: Wie viele unterschiedliche Wörter
  % zählt der nächste Satz, meinen Sie?
  %
  % \begin{quotation}
  %   Ich mag meinen Onkel, aber meine Tante mag mich nicht.
  % \end{quotation}
  %
  % Sind \textit{meinen} und \textit{meine} unterschiedliche Wörter?
  % Was ist mit \textit{ich} und \textit{mich}? Insbesondere wenn
  % man viele Texte hat, ist es zwar einfacher,
  % solche Wörter als unterschiedlich zu betrachten, aber ggf.\ büsst
  % man dann Datenqualität ein.
  %
  % \citet{Bonvin2018} analysierten 1'020 argumentative und narrative
  % deutsche Texte, die von Schulkindern mit und ohne portugiesischen
  % Hintergrund geschrieben wurden. Sie berechneten für jeden Text u.a.\
  % den Guiraud-Index mit zwei unterschiedlichen Berechnungsweisen---einer
  % einfach zu berechnenden und einer mühsam zu berechnenden, aber linguistisch sinnvolleren.
  % Lasst uns aber tun, als ob die Forschenden
  %
  % \begin{enumerate}
  % \item einlesen
  % \item subset selektieren
  % \item korrelation berechnen, interpretieren
  % \end{enumerate}
  %
  % \item
% \end{enumerate}
...
<<echo = FALSE, fig.cap = "Wenn man von Hand eine gerade Linie durch die Punktwolke ziehen würde, zeichnet jeder die Linie wohl an einer anderen Stelle. Wir können aber Kriterien festlegen, sodass alle die gleiche Linie zeichnen.\\label{fig:differentregressions}", fig.width = 4, fig.height = 2.4>>=
ggplot(data = d,
       aes(x = AOA,
           y = GJT)) +
  geom_point(shape = 1) +
...
einen Teil, der die Gemeinsamkeiten zwischen allen Werten ausdrückt,
und einen Teil, der die individuellen Unterschiede zwischen diesen
...
<<echo = FALSE, fig.cap = "Schnittpunkt und Steigung einer Geraden.\\label{fig:gerade}">>=
...
und unsere nächste Aufgabe ist es, diese Parameter so gut wie möglich zu \textbf{schätzen}.
Dass wir diese Parameter nur schätzen und nicht \emph{wissen} können, zeigt sich
...
aber uns interessieren nur die $\beta_0$- und $\beta_1$-Werte der \emph{optimalen} Geraden.
...
dann ist $$\sum_{i = 1}^{n} \widehat{\varepsilon_i}^2 = \sum_{i = 1}^{n} (y_i - (185 - 2x_i))^2 = 107121.$$
Das berechnet man in R so:
<<>>=
sum((d$GJT - (185 - 2*d$AOA))^2)
@

\item Sind  $\widehat{\beta_0} = 190$ und  $\widehat{\beta_1} = -1.5$,
dann ist $$\sum_{i = 1}^{n} \widehat{\varepsilon_i}^2 = \sum_{i = 1}^{n} (y_i - (190 - 1.5x_i))^2 = 28810.$$
In R:
<<>>=
...
$\widehat{\beta} = (190, -1.5)$ sind also optimaler
...
algebraisch finden kann, und zwar so:
...
Einheiten die Gerade steigt oder senkt, wenn \texttt{AOA}
...
<<fig.width = 4, fig.height = 2.3, fig.cap = "Ein Streudiagramm mit einer Regressionsgeraden. Die Regressionsgerade erfasst die modellierte zentrale Tendenz (genauer: das GJT-Mittel) für die unterschiedlichen AOA-Werte.\\label{fig:scatterwithreg}">>=
ggplot(data = d,
       aes(x = AOA,
           y = GJT)) +
  geom_point(shape = 1) +
...
liegen die $y_i$-Werte \emph{abzüglich} des Restfehlers, also
$$\widehat{y_i} = \widehat{\beta_0} + \widehat{\beta_1}x_i.$$

Wie man diese $\widehat{y_i}$-Werte \emph{konzeptuell} interpretieren
...
\section{Unsicherheit in Parameterschätzungen schätzen}
...
\begin{enumerate}\label{nonparametricbsregression}
 ...
 \item Schritte 2--4 werden ein paar tausend Mal ausgeführt, sodass man die Verteilung der bootstrapten $\beta$-Schätzungen erhält.
\end{enumerate}
...
<<echo = TRUE, fig.width = 6, fig.height = 2, fig.cap = "Verteilung der Bootstrap-Schätzungen der Parameter im Regressionsmodell \\texttt{aoa.lm}.\\label{fig:bootstrapdistributiondekeyser}">>=
...
\texttt{coef(aoa.lm)} gobt zwei Werte aus:
...
gezeigt; die entsprechenden Regressionsgeraden sieht man in Abbildung
...
<<fig.width = 6, fig.height = 1.4, echo = FALSE, fig.cap = "Regressionsgeraden, die anhand der 1., 2., 3.\\ und 4.\\ Bootstrapschätzungen des Schnittpunktes und der Steigung gezeichnet wurden. Die Grafiken sind einander recht ähnlich, aber nicht identisch.\\label{fig:bootstrapregressionline}">>=
plot1 <- ggplot(data = d,
                aes(x = AOA, y = GJT)) +
  geom_point(shape = 1) +
...
<<cache = TRUE, fig.width = 3, fig.height = 2, echo = TRUE, fig.cap = "Regressionsgeraden, die auf der Basis von 100 Bootstrapschätzungen des Schnittpunktes und der Steigung gezeichnet wurden.\\label{fig:bootstrapregressionline100}">>=
...
Anstatt das man diese Regressionsgeraden einzeln darstellt,
...
ist es praktischer, diese Werte in einer Matrize zu speichern
...
for (i in 1:runs) {
  ...
  # Werte in jeweils eine neue Zeile der Matrix
...
<<cache = TRUE, fig.width = 3, fig.height = 2, echo = TRUE, fig.cap = "Regressionsgerade mit 80\\%- und 95\\%-Konfidenzbändern, die mittels Bootstrapping berechnet wurden.\\label{fig:btstrpconfidenceband}">>=
...
\paragraph{Identisch und unabhängig verteilte Restfehler}
 ...
 \item Jemand möchte die durchschnittliche Länge von [\textipa{i:}]-Produktionen
 von Bernern schätzen und wählt zufällig 25 Berner aus. (So weit, so gut.)
 Jeder Sprecher liest 50 Wörter mit einem [\textipa{i:}] vor.
 ...
 \item Ausserdem ist es wahrscheinlich, dass das [\textipa{i:}] in bestimmten
 ...
 Wenn die sogenannte `i.d.d.'-Bedingung (\textit{identically and independently distributed})
 ...
 mit einer Wahrscheinlickheit von jeweils 50\% 1'000 Einheiten zum Mittel.
 ...
 Im Beispiel mit den [\textipa{i:}] könnten wir also \emph{nicht} den Standardfehler
...
<<fig.width = 4, fig.height = 1.4, echo = FALSE, fig.cap = "Die Streuung in beiden Streudiagrammen variiert erheblich je nach dem $x$- oder $\\widehat{y}$-Wert.\\label{fig:heteroskedasticity}", message = FALSE>>=
x <- runif(100, 0, 100)
y_hat <- 0.3*x
...
\subsection{Bootstrappen unter der Normalitätsannahme}
...
\subsection{Mit $t$-Verteilungen}\label{sec:aoa}
Wenn wir ohnehin davon ausgehen, dass die Residuen
(i.d.d.) normalverteilt sind, können wir den Standardfehler,
...
<<fig.width = 4.5, fig.height = 2.8, fig.cap = "Regressionsgerade mit 67\\%- und 95\\%-Konfidenzband. Konfidenzbänder von Regressionsmodellen sind übrigens am schmalsten beim durchschnittlichen x-Wert.\\label{fig:geomsmooth}", message = FALSE>>=
ggplot(data = d,
       aes(x = AOA,
           y = GJT)) +
  geom_point(shape = 1) +
  geom_smooth(method = "lm", level = 0.95, fill = "red") +
  geom_smooth(method = "lm", level = 0.67, fill = "darkred")
@
...
zufällig (und `i.d.d.') aus einer Verteilung mit Mittel 0 stammen.
...
Konzeptuell erlaubt uns die Annahme,
...
8 und 26 Jahren haben, ist es gefährlich, Aussagen über 5- oder 40-Jährige zu machen.
 ...
 Auch bei \textbf{Intrapolation} ist Vorsicht geboten.
...
<<echo = FALSE, fig.width = 6, fig.height = 2.5>>=
...
Übrigens ist das 95\%-Konfidenzintervall um das Stichprobenmittel
dank der Berücksichtigung des Zusammenhangs zwischen AOA und GJT
auch schmaler geworden: $[147.0, 154.5]$
verglichen mit $[144.5, 157.0]$ vorher.
...
\paragraph{Denkfrage} Obwohl die Modelldiagnose
nicht auf grössere Probleme hindeutet, \emph{können}
...
\begin{enumerate}
 \item Führen Sie folgende Analyse auf die \rcode{dekeyser2010.csv}-Daten aus:
<<eval = FALSE>>=
plot(AOA ~ GJT, data = d)
...
\end{enumerate}
 ...
 den Spalten \texttt{CorrectWritten} bzw.\ \texttt{CorrectSpoken}. Die Datei \texttt{vanhove2014\_background.csv}
