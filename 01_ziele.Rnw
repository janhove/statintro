...
\begin{itemize}
  ...
  um die Ohren gehaut. Die meisten von denen haben aber keine
  Relevanz für die Forschungsfrage, sodass man als LeserIn zuerst
  die relevanten von den irrelevanten Informationen trennen muss---eine
  Verantwortung, die ja eigentlich bei den AutorInnen liegt. Ich vermute,
  dass der Überfluss von sinnlosen Informationen der Tatsache
  ...
  Verfahren als Ziel an sich zu sein. 
...
gedacht.
Trotzdem empfiehtl es sich, sich irgendwann mit diesen Praktiken
auseinanderzusetzen (siehe dazu Kapitel \ref{ch:sinnlos} und \ref{ch:QRP}
sowie viele der Literaturempfehlungen).
...
liefern wird. 
% Ressourcen, die Ihnen dabei helfen können, Forschungsfragen
% besser zu formulieren, kenne ich momentan keine, aber sobald ich etwas
% Nützliches gefunden oder entwickelt habe, werde ich es auf meinen Blog stellen.
...
Der Fokus des Skripts liegt auf dem Schätzen
 ...
 Teil \ref{pt:stichproben} (`Stichproben und Populationen',
 Kapitel \ref{ch:descriptives} bis \ref{ch:uncertainty})
 legt dafür die Fundamente.
 ...
 klarer darlegen als eine rein mathematische Erklärung dies würde.
 
 In Teil \ref{pt:lm} (Kapitel \ref{ch:linmod} bis \ref{ch:multimod})
 ...
 diese daher erst in Teil \ref{pt:signifikanz}
 (Kapitel \ref{ch:logik} bis \ref{ch:QRP}), unter Begleitung vieler Wenn
 und Aber, besprochen.
 
 Die Kapitel in Teil \ref{pt:beyond} geben Ihnen Empfehlungen,
 wo Sie sich über komplexere Verfahren, die m.E.\ nicht zu den Grundlagen
 gehören, schlau machen können.
 
 Im Anhang ab Seite \pageref{pt:anhang} finden Sie eine Erklärung der
...
Weiter muss ich von Neugier und Eigeninitiative ausgehen, weil
...
Für jegliche inhaltliche und sprachliche Hinweise bin ich dankbar.

Danke an Isabelle Udry, die mich auf etliche sprachliche Fehler
und inhaltliche Unklarheiten in einer der neueren Versionen
hingewiesen hat. Diese habe ich dann prompt durch andere ersetzt. 

\bigskip

Viel Erfolg, Mut und Spass!
% 
% \begin{flushright}
% Jan Vanhove\\
% Freiburg, Schweiz, Juli 2021
% \end{flushright}