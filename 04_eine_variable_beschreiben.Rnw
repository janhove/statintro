...
Für diese Arbeit
habe ich 23 Studierenden im zweiten Jahr im
Fach schwedische Sprach- und Literaturwissenschaft
an der Universität Gent vier
Leseverstehensaufgaben vorgelegt:
...
und das \texttt{here}-Package geladen sind.
<<message = FALSE>>=
...
<<echo = FALSE, fig.cap = "Ein Punktdiagramm sortiert nach den IDs der Versuchspersonen (links) und eins geordnet nach dem Ergebnis (rechts). Es gibt keine Werte, die weit von anderen liegen.\\label{fig:dotchart}", fig.width = 6, fig.height = 3>>=
p1 <- ggplot(data = d,
       aes(x = Norwegian,
           y = Participant)) +
  geom_point() +
...
Um die linke Grafik zu zeichnen, können Sie diesen Befehl
verwenden. Achten Sie darauf, dass das \texttt{tidyverse}-Bündel
...
Die Kommentare erläutern, wie die Grafik aufgebaut wird.
<<eval = FALSE>>=
...
Achten Sie auf Gross- und Kleinschreibung, auf die Klammern
...
<<eval = FALSE>>=
ggplot(data=d,aes(x=Norwegian,
y=Participant))+geom_point()+
...
aufgrund von dem, was es eben \emph{nicht} aufzeigt. Es scheint
...
Ausreisser \emph{können} (nicht \emph{müssen}) auch auf technische
...
<<echo = FALSE, fig.cap = "Beispiel eines Ausreissers. Hier müsste man kontrollieren, ob der Datenpunkt (17.6) richtig eingetragen wurde und nicht etwa ein Tippfehler ist (statt 1.76).\\label{fig:outlier}">>=
...
In den Histogrammen bisher stand die \emph{Anzahl}
...
<<fig.cap = "Drei Histogramme mit der \\texttt{Norwegian}-Ergebnissen. Statt der absoluten Anzahl Beobachtungen pro \\textit{bin} stehen hier Wahrscheinlichkeitsdichten auf der y-Achse.\\label{fig:histogramdensity}", echo = FALSE, fig.width = 8.5, fig.height = 2.2>>=
grenzen <- seq(from = 0, to = 16, by = 1)
p1 <- ggplot(data = d,
       aes(x = Norwegian,
           y = ..density..)) +
...
\textbf{Gross-Sigmanotation.} Formeln wie diese mögen abschrecken,
...
Die Notation \texttt{d\$Norwegian[[4]]} identifiziert den 4.\ Wert eines Vektors oder einer Liste. Statt der Notation `[[' werden Sie auch oft `[' antreffen. Der Unterschied ist hier nicht so wichtig, aber mit `[[' identifiziert man immer \emph{einen} Wert; mit `[' kann man auch mehrere Werte gleichzeitig abfragen.
...
wie Java und Python verweisen aufs erste Element mit dem Index 0.
\end{framed}
...
nicht, aber sie zeigt, wie die \texttt{summarise()}-Funktion
funktioniert.
<<>>=
d %>%
...
Eine Modusfunktion gibt es nicht, aber wir können für jeden Wert
zählen, wie oft er vorkommt:
<<>>=
d %>%
  count(Norwegian)
@
...
Die folgenden Mittelwerte trifft man seltener an,
aber sie seien hier der Vollständigkeit halber erwähnt.

\paragraph{Das getrimmte Mittel}
Man löscht die $x\%$ kleinsten und die $x\%$ grössten Werte
und berechnet dann das Mittel. Fürs 25\% getrimmte Mittel
...
und basiert es auf nur zwei der Beobachtungen.
...
$\mu$ wird berechnet als $\frac{1}{N}(x_1 + x_2 + \dots + x_N)$,
...
Wäre es nicht einfacher, mit den \emph{absoluten} Unterschieden zu rechnen?
...
Aber eigentlich
sollte das Ergebnis \emph{genau} 0 sein, nicht \emph{fast} 0.
...
was unerwünscht ist. Die Lösung liegt auf der Hand: $d^2$ teilen durch die Anzahl Beobachtungen. Dies ergibt die Populationsvarianz ($\sigma^2$):
\begin{equation}
...
@
Achtung!
In der Regel müssen wir die Varianz einer Stichprobe, \emph{nicht} jene einer
...
kann man nicht absolut interpretieren:
...
Stichprobe, \emph{nicht} jene einer
Population berechnen. Diese wird leicht anders berechnet;
siehe Kapitel \ref{ch:stichproben}.
...
<<fig.cap = "Hinter dem gleichen Mittelwert kann sich eine Vielzahl von unterschiedlichen Mustern verstecken. Diese drei Grafiken zeigen alle 39 Beobachtungen einer Variablen mit einem Mittel von 2.43 (nach Rundung).\\label{fig:samemean}", echo = FALSE, fig.width = 6, fig.height = 2>>=
...
<<fig.cap = "Sogar wenn man das Mittel und die Standardabweichung kennt, weiss man noch nicht, welches Muster sich hinter diesen Zahlen versteckt. In all diesen Grafiken beträgt das Mittel nach Rundung 2.43 und die Standardabweichung 1.05.\\label{fig:samemeansd}", echo = FALSE, fig.width = 6, fig.height = 4>>=
...
\begin{framed}
Stellen Sie Ihre Daten auch grafisch dar, sodass Sie und Ihre Leserschaft
...
<<fig.cap = "Vier Histogramme der gleichen feinkörnigen Variablen.\\label{fig:density1}", fig.width = 8, fig.height = 2, echo = FALSE>>=
n <- 100000
...
<<fig.cap = "Eine Kerndichteschätzung der gleichen Variablen wie in Abbildung \\ref{fig:density1}.\\label{fig:density2}", fig.width = 2, fig.height = 1.5, echo = FALSE>>=
ggplot(df, aes(x = x)) +
  geom_density() +
    ylab("Wsk.-Dichte")
@
...
<<fig.cap = "Wahrscheinlichkeitsdichten von drei kontinuierlichen Gleichverteilungen.\\label{fig:uniform}", fig.width = 8, fig.height = 2, echo = FALSE>>=
...
<<fig.cap = "Die Form einer Normalverteilung ist von zwei Parametern abhängig: ihrem Mittel ($\\mu$) und ihrer Standardabweichung ($\\sigma$).\\label{fig:normal}", echo = FALSE, fig.width = 8, fig.height = 2>>=
...
ist eine nicht-symmetrische Verteilung, die nach rechts neigt.
...
<<fig.cap = "Eine bimodale und zwei schiefe Verteilungen.\\label{fig:other}", echo = FALSE, fig.width = 8, fig.height = 2>>=
...
@
  ...
  üblicherweise mit dem Medianen statt mit dem Mittel ausgedrückt. Warum?
