...
\section{Datensatz}
\citet{DeKeyser2010} untersuchten, wie das Alter,
...
\paragraph{Aufgabe 1}
Lesen Sie den Datensatz \texttt{dekeyser2010.csv} in R ein.
Zeichnen Sie die Grafik in Abbildung \ref{fig:gjthistogram} selbst.

\paragraph{Aufgabe 2}
Berechnen Sie das Mittel der GJT-Werte.

<<fig.cap = "Histogramm der GJT-Daten aus der Nordamerika-Studie von \\citet{DeKeyser2010}. Diese Grafik sollten Sie selber zeichnen (Aufgabe 1).\\label{fig:gjthistogram}", echo = FALSE, fig.width = 4, fig.height = 2>>=
ggplot(data = d,
       aes(x = GJT)) +
...
Verteilung der Mittel von 20'000 Stichproben ($n = 20$) aus
der Population.
...
p_bs <- ggplot(df, aes(x = means, y = ..density..)) +
  ...
  xlab("bootstrapte Mittel") +
  ylab("Wsk.-Dichte") +
  ggtitle("Verteilung der bootstrapten Mittel")
...
Das Mittel der bootstrapten Mittel ist gleich dem Mittel
der Stichprobe (145.95).
Ihre Standardabweichung beträgt etwa 5.20.
2.5\% der bootstrapten Mittel sind kleiner als 135.95;
...
Der Bootstrap ist eine ziemlich neue Technik,
...
Verteilung der bootstrapten Mittel für die fünf Stichproben;
...
<<echo = FALSE, cache = TRUE, warning = FALSE, fig.cap = "Die Verteilung der bootstrapten Mittel auf der Basis von fünf Stichproben.\\label{fig:bootstrapdistributions}", echo = FALSE, fig.width = 3, fig.height = 8>>=
...
und die fünf Verteilungen der bootstrapten Mittel. Die Perzentile und der Unterschied zwischen ihnen wurden gerundet.}
\label{tab:bootstrap}
\begin{tabular}{lrrrr}
\toprule
...
\begin{itemize}
 ...
 Verteilungen der bootstrapten Mittel zwar
...
ist es ja kaum sinnvoll, kleinere Stichprobe aus ihr zu ziehen.
...
die Standardabweichung der bootstrapten Mittel.
...
Berichten würde ich die Schätzung des Mittels und die Unsicherheit über diese Schätzung als $150.8 \pm 3.1$ oder sogar $151 \pm 3$.
...
Etwa 95\% der bootstrapten Mittel liegen
...
Da die Verteilung der bootstrapten Mittel in etwa
...
Dies gilt natürlich nur, wenn die Verteilung der bootstrapten
...
Dazu brauchen Sie lediglich eine Zeile des Codeabschnitts
...
Was fällt Ihnen verglichen mit den vorigen Übungen auf? Wenn Ihnen nichts auffällt, sollten Sie
die Anzahl \textit{bins} im Histogramm vergrössern: \texttt{hist(bootstraps, breaks = 100)}. Können Sie sich Ihren Befund
erklären?

\section{Das \textit{plug-in}-Prinzip und der zentrale Grenzwertsatz}
In der Praxis wird der Bootstrap
eher selten verwendet, um die Unsicherheit eines
...
Stichprobenmittel ($\bar{x}$) zu einer Normalverteilung neigen,
...
Wenn wir (a) annehmen wollen, dass der zentrale Grenzwertsatz
bereits bei unserer Stichprobengrösse zutrifft, und (b)
...
Wenn die Standardabweichung der Population uns nicht bekannt ist,
...
Beobachtungen noch nicht zutrifft. Wenn dieser Verdacht besteht,
wäre der Bootstrap also geeigneter. (Man sollte bei solchen
Verteilungen aber ohnehin einmal überlegen, ob man sich
wirklich für ihr Mittel interessieren sollte; siehe
\href{https://janhove.github.io/analysis/2019/04/11/assumptions-relevance}{\textit{Before worrying about model assumptions, think about model relevance}}.)
...
Meistens ist es unmöglich, $s$ für diese Verzerrung zu korrigieren.
...
Abbildung \ref{fig:tdistribution} zeigt eine Standardnormalverteilung und ein paar $t$-Verteilungen.
...
$t$-Ver\-teil\-ung\-en mit 2, 5 und 25 Freiheitsgraden. Je mehr Freiheitsgrade,
...
für die den zentralen Grenzwertsatz bei dieser Stichprobengrösse zutrifft,
