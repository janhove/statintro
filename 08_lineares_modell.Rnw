...
mit der wir uns in diesem Kurs nur etwas befassen; siehe Kapitel \ref{ch:logistic}.}
...
Oft redet man statt von Prädiktoren und \textit{outcome}
...
in zwei Teile zu zerlegen: einen Teil, der die
Gemeinsamkeiten zwischen allen Werten ausdrückt,
und einen Teil, der die individuellen Unterschiede
...
$y_i$ ist die i.\ Beobachtung im Datensatz,
...
In der Regel interessieren wir uns am stärksten
für $\beta_0$. Da uns aber nicht die ganze Population
...
auf. Die ersten beiden GJT-Werte im Datensatz sind
151 und 182. Wenn wir für $\beta_0$ einen beliebligen
Wert, z.B.\ 1823, wählen,
gilt $\widehat{\varepsilon_1} = -1672$ und $\widehat{\varepsilon_2} = -1641$ und die Gleichung geht auf:
$$y_1 = 151 = 1823 - 1672$$
$$y_2 = 182 = 1823 - 1641$$
Wenn wir für $\beta_0$ den
Wert 14 wählen, gilt $\widehat{\varepsilon_1} = 137$ und $\widehat{\varepsilon_2} = 168$
...
werden grosse Residuen mehr bestraft.
...
Algebraisch ist die Methode der kleinsten Quadrate am einfachsten,
...
in den Sprachwissenschaften ist dies aber eher selten der Fall.
Im Prinzip kann man sogar selber beliebige Optimierungskriterien
...
Es handelt sich um die $y$-Werte \emph{abzüglich} der $\hat{\varepsilon}$-Werte: $\widehat{y_i} = y_i - \widehat{\varepsilon_i}$.
...
aus der Stichprobe zu ziehen, ziehen wir nur Bootstrap-Stichproben
aus den Residuen ($\varepsilon$). Diese kombinieren wir dann
mit $\widehat{\beta_0}$, um die Bootstrap-Stichproben zu kombinieren.
...
\begin{enumerate}\label{bootstrapoverview}
 \item Man berechnet $\widehat{\beta_0}$ und erhält dazu auch noch $\hat{\varepsilon}$.
 ...
 der bootstrapten $\beta_0$-Schätzungen.
\end{enumerate}
...
bootstrapten Parameterschätzungen visualisieren,
...
Unserer Annahme zufolge gäbe es in der Population also
...
der Restfehler in der Stichprobe \emph{geschätzt}.
...
\widehat{\sigma_{\varepsilon}} = \sqrt{\frac{1}{n - p} \sum_{i = 1}^{n} \widehat{\varepsilon_i}},
\end{equation}
wo $p$ die
...
Der Bootstrap aus dem letzten Abschnitt -- bei der man
...
Der Bootstrap aus dem letzten Kapitel -- bei der man
...
Wenn man ohnehin davon ausgeht, dass die Residuen
normalverteilt sind, können der geschätzte
Standardfehler und das Konfidenzintervall
auch algebraisch hergeleitet werden.
...
Das 95\%-Konfidenzintervall kann anhand von \texttt{confint()}
berechnet werden.
