...
Stellen Sie sich folgendes Experiment
vor (inspiriert von \citealp{Guiora1972}).
...
<<echo = FALSE, fig.width = 3, fig.height = 3, fig.cap = "Ergebnisse eines fiktiven Experiments.\\label{fig:experiment}">>=
...
Ergebnisse nicht \emph{systematisch} verzerrt wurden.
...
Randomisierung ist eben \emph{nicht}, perfekt
...
kann sich ausgewälte Kapitel aus \citealp{Oehlert2010}, anschauen)
oder indem wir die Sprachgeschwindigkeit der Versuchspersonen auch
vor dem Experiment gemessen hätten (`Vortest'), sodass wir diese
...
In der `frequentistischen' Tradition des Nullhypothesen Testens
...
und Kontrollgruppe hätten aussehen können. Dies können wir
...
<<echo = FALSE, fig.width = 10, fig.height = 13, fig.cap = "Die 20 möglichen Ergebnisse laut der Annahme, dass die Unterschiede in der Stichprobe nur der Randomisierung zuzuschreiben sind.\\label{fig:20moeglichkeiten}">>=
...
@
% 
% % Die gleiche Lösung erhält man mit der Funktion \texttt{independence\_test()}
% % aus dem \texttt{coin}-Paket.
% % 
% % <<message = FALSE>>=
% % geschwindigkeit <- c(4.2, 3.8, 5.0, 3.1, 3.4, 4.3)
% % kondition <- factor(c(rep("ohne Alkohol", 3), rep("mit Alkohol", 3)))
% % 
% % # ggf. 'coin' installieren
% % library(coin)
% % independence_test(geschwindigkeit ~ kondition, distribution = "exact")
% % @
% 
...
weiter den Unterschied zwischen \textbf{zufällige Zuordnung}
und \textbf{zufällige Auswahl}: Wir haben unsere Versuchspersonen 
...
es ist wichtig zu erkennen, dass es keine statistische Frage ist.
...
%  dass der beobachtete Gruppenunterschied nur durch Zufall zu Stande gekommen
%  ist, generiert.
%  
...
%   Ergebnis in einer Replikationsstudie bestätigen würde.\footnote{Ich habe keine
...
% `statistisch nicht-signifikanten' $p$-Werten. Dabei gelten $p$-Werte
...
% zählt.\footnote{Neuerdings gibt es, insbesondere auf den sozialen Medien,
...
% festgelegt: Nur 5\% aller statistischen Nullhypothesen
...
kann man \texttt{distribution = approximate()}; dann werden
defaultmässig `nur' 10'000 zufällige Permutationen generiert. 
Hier ein Beispiel
...
Also 0.33\% ($p = 0.0033$). Wenn Sie diese Berechnungen
...
bei Ihnen dann eben nicht die gleichen wie bei mir sind.
...
Also 0.69\% ($p = 0.0069$). Die Wahrscheinlichkeit,
...
sind, deren Annahme sich insbesondere in kontrollierten Experimenten
...
Anno dazumal war es zu aufwändig, ein paar tausend
...
von extremeren als dem beobachteten $t$-Wert zu berechnen
...
\begin{framed}
\textbf{Ein $t$-Test ist ein lineares Modell.} Und zwar eins,
...
ist ein Basisprinzip des experimentellen Forschungsdesigns.
...
den Kapiteln in Teil III finden Sie hierfür viele Beispiele:

\begin{itemize}
 \item Seite \pageref{sec:aoa}: Die Nullhypothesen, die überprüft werden, besagen, dass die Parameter für \texttt{(Intercept)} und \texttt{AOA} eigentlich gleich 0 sind. Bemerken Sie, dass diese erste Nullhypothese für niemanden von Interesse ist, da man sich kaum fürs Intercept interessiert und da niemand behaupten würde, er dürfte gleich 0 sein. 
 ...
 % \item Seite \pageref{sec:kognat}.

 % \item Seite \pageref{sec:alles}.
